...
function parseNameAndRest(args) {
	const first = consumeLeadingValue(args.join(" ").trim());
	if (!first) {
		return { name: "", rest: "" };
	}
...
function parseTwoNames(args) {
	...
	return { firstName: first.value, secondName: second?.value || "" };
}
...
async function resolveAddTarget(ctx, target) {
	...
	if (/youtube\.com|youtu\.be/.test(input)) {
		if (/[&?]list=|\/playlist/.test(input)) {
			...
			return videos.map((v) => buildYouTubeQueueItem(v, "audio"));
		}
